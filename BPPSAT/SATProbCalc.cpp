...
void SolveSATbpp(int n, double * SAT, int c, unsigned long long int i){//number of variables, SAT instance, number of clauses, number of iterations
    srand(time(NULL));
    vector<char> s(n,'0');
    ...
    for (unsigned long long int it = 0;it < i;it++){
        solved = true;
        for(int t = 0; t < c ; t++){
            ...
            &&  ((s[(abs((int)SAT[3*t+2])-1)] == '1' && (SAT[3*t+2]>0)) || (s[(abs((int)SAT[3*t+2])-1)] == '0' && (SAT[3*t+2]<0)) ) ){
                // cout<< "IN HERE" << (rand()%2) << endl;
                s[(abs((int)SAT[3*t])-1)] = '0' + (rand()%2);//static_cast<char>((rand()%2));
                s[(abs((int)SAT[3*t+1])-1)] = '0' + (rand()%2);//static_cast<char>((rand()%2));
                s[(abs((int)SAT[3*t+2])-1)] = '0' + (rand()%2);//static_cast<char>((rand()%2));
                solved = false;
                // t = 0;
            }
